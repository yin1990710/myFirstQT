...
def plot_chart(data):
    ...
    for i, (date, close, tag) in enumerate(zip(dates, close_values, turning_points)):
        if tag and tag in tag_colors:
            ax1.scatter(date, close, color=tag_colors[tag], marker=tag_markers[tag], s=120, zorder=5)
            ax1.annotate(tag, (date, close), textcoords="offset points", xytext=(0, 12), ha='center', fontsize=9, fontweight='bold')

    ax1.set_xlabel('日期', fontsize=12)
    ax1.set_ylabel('价格', fontsize=12)
    ax1.set_title('300373.SZ MA5走势与转折点标记', fontsize=16, fontweight='bold')
    ...
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '300373v4.png')
