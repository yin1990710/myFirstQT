...
def analyze_stocks(data):
    ...
    for ts_code, info in stock_data.items():
        ...
        closes_200 = [r['close'] for r in recent_200]
        min_close = min(closes_200)
        max_close = max(closes_200)
        
        if max_close == 0:
            continue
        
        if (min_close / max_close) > 0.5:
            continue
        ...
        a1_slope = (recent_ma5[day1_idx] - recent_ma5[day10_idx]) / (day1_idx - day10_idx)
        b1_slope = (recent_ma30[day1_idx] - recent_ma30[day10_idx]) / (day1_idx - day10_idx)

        a1_start = (day10_idx, recent_ma5[day10_idx])
        a1_end = (day1_idx, recent_ma5[day1_idx])
        b1_start = (day10_idx, recent_ma30[day10_idx])
        b1_end = (day1_idx, recent_ma30[day1_idx])

        x = calculate_angle(a1_start, a1_end, b1_start, b1_end)

        if a1_slope < 0 or b1_slope < 0 or x < 0:
            continue

        if not lines_intersect(a1_start, a1_end, b1_start, b1_end):
            continue
...
def generate_csv_file(stocks, folder_path):
    csv_filename = "bottom_up.csv"
    csv_path = os.path.join(folder_path, csv_filename)

    with open(csv_path, 'w', newline='', encoding='utf-8-sig') as f:
        writer = csv.writer(f)
        writer.writerow(['股票代码', '股票名称', '夹角x', 'a1斜率', 'b1斜率'])
        for stock in stocks:
...
def main():
    ...
    if selected_stocks:
        ...
        for stock in selected_stocks:
            print(f"• {stock['ts_code']} - {stock['stock_name']} | x={stock['x']:.2f}° | a1={stock['a1_slope']:.4f} | b1={stock['b1_slope']:.4f}")
    else:
        print("\n" + "=" * 80)
        print("⚠️ 没有满足条件的股票")
        print("=" * 80)
