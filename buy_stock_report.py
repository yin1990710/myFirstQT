...
INDEX_CODE = '932078.CSI'

def get_report_dir() -> str:
    """
    获取报告目录路径（带日期后缀）

    :return: 目录路径，格式为 买入股品字定向分析报告_YYYYMMDD
    ...
    return os.path.join(os.getcwd(), f'买入股股品字定向分析报告_{target_date.strftime("%Y%m%d")}')
...
def get_table_name() -> str:
    """
    获取带日期后缀的win_win_stocks_t表名

    :return: 表名，格式为 win_win_stocks_t_YYYYMMDD
    如果当前时间为0-18点，则取前一天的日期
    """
    now = datetime.now()
    hour = now.hour
    ...
    return f"win_win_stocks_t_{target_date.strftime('%Y%m%d')}"
...
def get_win_win_stocks(conn) -> list:
    """
    从win_win_stocks_t_YYYYMMDD表获取所有股票代码列表
...
def get_index_daily_data(conn, ts_code: str = None) -> pd.DataFrame:
    """
    从index_daily_t表获取指数日线数据

    :param conn: 数据库连接
    :param ts_code: 指数代码，默认使用中证全指
    ...
    try:
        ...
        print(f"⚠️ 获取指数 {ts_code} 日线数据失败: {e}")
        return pd.DataFrame()

def calculate_risk_opportunity_index(df: pd.DataFrame) -> dict:
    """
    计算行情风险指数和行情机会指数

    :param df: 指数日线数据（最近30个交易日）
    :return: 包含两个指数信息的字典
    """
    if len(df) < 30:
        return {
    ...
    pct_chg_30d = ((end_price - start_price) / start_price) * 100
    ...
    return {
        'risk_index': risk_stars,
        'risk_stars': '★' * risk_stars + '☆' * (5 - risk_stars) if risk_stars > 0 else '无数据',
        'opportunity_index': opportunity_stars,
        'opportunity_stars': '★' * opportunity_stars + '☆' * (5 - opportunity_stars) if opportunity_stars > 0 else '无数据',
        'pct_chg_30d': pct_chg_30d
    }

def generate_index_chart(df: pd.DataFrame, index_info: dict, output_path: str):
    """
    生成中证全指K线图，包含行情风险指数和机会指数

    :param df: 指数日线数据
    :param index_info: 指数信息（包含风险指数和机会指数）
    ...
    for idx, row in df.iterrows():
        ...
        ax1.plot([trade_date, trade_date], [low_price, high_price], color=color, linewidth=0.8)
        ax1.add_patch(plt.Rectangle((trade_date - width/2, body_bottom), width, body_height if body_height > 0 else 0.1,
                                   edgecolor=color, facecolor=color, linewidth=0.8))

    ax1.set_ylabel('价格', fontsize=12)
    ax1.grid(True, alpha=0.3, linestyle='--')
    ax1.set_title(f'中证全指（{INDEX_CODE}）走势K线图', fontsize=14, pad=10)

    ax1.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
    ax1.xaxis.set_major_locator(mdates.WeekdayLocator(interval=1))
    plt.setp(ax1.xaxis.get_majorticklabels(), rotation=45, ha='right')

    risk_label = f"行情风险指数: {index_info['risk_stars']} ({index_info['risk_index']}星)"
    opp_label = f"行情机会指数: {index_info['opportunity_stars']} ({index_info['opportunity_index']}星)"

    ax2.text(0.5, 0.7, risk_label, fontsize=16, ha='center', va='center',
             fontweight='bold', color='#dc3545',
             bbox=dict(boxstyle='round', facecolor='#fff3cd', edgecolor='#dc3545', alpha=0.9))
    ax2.text(0.5, 0.3, opp_label, fontsize=16, ha='center', va='center',
             fontweight='bold', color='#28a745',
             bbox=dict(boxstyle='round', facecolor='#d4edda', edgecolor='#28a745', alpha=0.9))

    if index_info['pct_chg_30d'] != 0:
        pct_text = f"近30日涨跌幅: {index_info['pct_chg_30d']:+.2f}%"
        ax2.text(0.5, 0.0, pct_text, fontsize=12, ha='center', va='center', color='#666')
...
def get_stock_daily_data(conn, ts_code: str) -> pd.DataFrame:
    """
    获取股票日线数据（从win_win_stocks_t_YYYYMMDD表）
...
def generate_html_report(stock_info: dict, df: pd.DataFrame, valley_info: dict,
                        ...
                        index_chart_path: str = None, index_info: dict = None):
    """
    生成HTML格式的分析报告
    ...
    :param output_path: 输出路径
    :param index_chart_path: 中证全指K线图路径
    :param index_info: 中证全指指数信息（包含风险指数和机会指数）
    """
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            {" ".format(
                f'''
            <div class="section">
                <div class="section-title">📈 中证全指（{INDEX_CODE}）走势及行情分析</div>
                <div class="chart-container">
                    <img src="{os.path.basename(index_chart_path)}" alt="中证全指K线图">
                </div>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 20px;">
                    <div style="background: #fff3cd; border-radius: 10px; padding: 15px; border-left: 4px solid #dc3545;">
                        <h4 style="color: #dc3545; margin-bottom: 10px;">📊 行情风险指数定义</h4>
                        <ul style="font-size: 12px; line-height: 1.8; padding-left: 20px; margin: 0;">
                            <li><strong>★★★★★ 5星</strong>: 近30个交易日涨幅超7%</li>
                            <li><strong>★★★★☆ 4星</strong>: 近30个交易日涨幅5%-6.99%</li>
                            <li><strong>★★★☆☆ 3星</strong>: 近30个交易日涨幅3%-4.99%</li>
                            <li><strong>★★☆☆☆ 2星</strong>: 近30个交易日涨幅2%-2.99%</li>
                            <li><strong>★☆☆☆☆ 1星</strong>: 近30个交易日涨幅2%以内</li>
                        </ul>
                        <p style="font-size: 11px; color: #666; margin-top: 10px;">💡 风险指数越高，表示市场近期涨幅较大，回调风险增加</p>
                    </div>
                    <div style="background: #d4edda; border-radius: 10px; padding: 15px; border-left: 4px solid #28a745;">
                        <h4 style="color: #28a745; margin-bottom: 10px;">🎯 行情机会指数定义</h4>
                        <ul style="font-size: 12px; line-height: 1.8; padding-left: 20px; margin: 0;">
                            <li><strong>★★★★★ 5星</strong>: 近30个交易日跌幅超9%</li>
                            <li><strong>★★★★☆ 4星</strong>: 近30个交易日跌幅7%-8.99%</li>
                            <li><strong>★★★☆☆ 3星</strong>: 近30个交易日跌幅5%-6.99%</li>
                            <li><strong>★★☆☆☆ 2星</strong>: 近30个交易日跌幅3.01%-4.99%</li>
                            <li><strong>★☆☆☆☆ 1星</strong>: 近30个交易日跌幅3%以内</li>
                        </ul>
                        <p style="font-size: 11px; color: #666; margin-top: 10px;">💡 机会指数越高，表示市场近期跌幅较大，潜在机会增加</p>
                    </div>
                </div>
            </div>
                ''' if index_chart_path and os.path.exists(index_chart_path) else ''
            )}
...
def create_stock_report(conn, ts_code: str, report_dir: str,
                       index_chart_path: str = None, index_info: dict = None) -> bool:
    """
    为单个股票生成分析报告
    ...
    generate_html_report(stock_info, df, valley_info, chart_path, html_path,
                        index_chart_path, index_info)
...
def main():
    ...
    try:
        ...
        print("\n📈 步骤4: 获取中证全指数据并生成K线图")
        index_df = get_index_daily_data(conn)
        index_chart_path = None
        index_info = None

        if not index_df.empty:
            index_info = calculate_risk_opportunity_index(index_df)
            now = datetime.now()
            hour = now.hour
            if 0 <= hour < 18:
                target_date = now - timedelta(days=1)
                ...
                target_date = now
            today = target_date.strftime('%Y%m%d')
            index_chart_path = os.path.join(report_dir, f'中证全指_{today}_K线图.png')
            generate_index_chart(index_df, index_info, index_chart_path)
            print(f"   ✅ 中证全指K线图已生成")
            print(f"   📊 行情风险指数: {index_info['risk_stars']} ({index_info['risk_index']}星)")
            print(f"   📊 行情机会指数: {index_info['opportunity_stars']} ({index_info['opportunity_index']}星)")
        else:
            print(f"   ⚠️ 未获取到中证全指数据，跳过K线图生成")
