...
核心功能：
  根据输入的目标股票代码，从 stock_daily_t 表读取所有股票最近 10 个交易日的 MA5 序列
  和成交量序列，找出与目标股票综合相似度最高的前 10 只股票。

计算方法：
  1. 取窗口长度 N=10，提取目标股票A、对比股票B 近 10 日 MA5 序列、成交量序列
  2. 分别做 MinMax 归一化
  3. 计算归一后两个序列的 DTW 距离 d
     （DTW 累积平方距离除以对齐步数取根号 —— 即逐步RMS偏差，
       使 d 落在 [0,1] 量级：两序列完全一致时 d=0，逐点偏差为1时 d=1）
  4. 换算得分：score = 1/(1+d)
  5. 5日均线相似度 与 成交量相似度 均按上述方法计算
  6. 最终相似度 = 5日均线相似度 × 成交量相似度，按最终相似度降序取前 10 只
...
def find_similar(target_code, data, trade_dates, window, top_n=10):
    """计算全市场与目标股票的最终相似度（MA5×成交量），返回最终相似度降序的前 top_n 只。
    ...
    cnt_data_missing = 0  # 近 N 日 MA5/成交量 数据不全
    ...
    vol_a = extract_series(target_code, 'vol')
    if ma5_a is None or vol_a is None:
        print(f"❌ 目标股票 {target_code} 近{window}个交易日 MA5/成交量 数据不全，无法比较")
        return []
    ...
    for ts_code in stock_data:
        ...
        vol_b = extract_series(ts_code, 'vol')
        if ma5_b is None or vol_b is None:
            cnt_data_missing += 1
            continue

        # 5日均线相似度
        ma5_score, ma5_d = similarity_score(ma5_a, ma5_b)
        # 成交量相似度（方法同MA5：MinMax归一化 + DTW）
        vol_score, vol_d = similarity_score(vol_a, vol_b)
        # 最终相似度 = 5日均线相似度 × 成交量相似度
        final_score = ma5_score * vol_score
    ...
    print(f"窗口长度: N={window} 个交易日（MA5 + 成交量，MinMax归一化 + DTW）")
    print("评分公式: 最终相似度 = 5日均线相似度 × 成交量相似度")
    print("         （每项 score = 1/(1+d)，d 为归一化序列的DTW距离）")
    print("-" * 40)
    print(f"  股票总数量:                       {total}")
    for name, cnt in cnt_filtered.items():
        print(f"  - 过滤[{name}] 淘汰: {cnt}")
    print(f"  - 近{window}日MA5/成交量数据不全 淘汰: {cnt_data_missing}")
    print(f"  - 目标股票自身:                   1")
    print("-" * 40)
...
def main():
    ...
    print("=" * 80)
    print(f"  目标股票: {target_code}")
    print(f"  算法: 近{window}日 MA5/成交量 → MinMax归一化 → DTW距离 d → score=1/(1+d)")
    print("  最终相似度 = 5日均线相似度 × 成交量相似度（无阈值过滤）")
    ...
    for i, s in enumerate(top_stocks, 1):
        print(f"{i:>2}. {s['ts_code']:<11} 最终={s['final_score']:.4f}  "
              f"(MA5={s['ma5_score']:.4f} × 量={s['vol_score']:.4f})  "
              f"d_ma5={s['ma5_d']:.3f} d_vol={s['vol_d']:.3f}")
