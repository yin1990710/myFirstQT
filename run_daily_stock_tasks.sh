...
# 步骤1：指数日交易数据
log "[步骤1/25] 开始执行 指数日交易数据 (update_stock_index_daily.py)..."
if ${VENV_PYTHON} update_stock_index_daily.py >> "${LOG_DIR}/update_stock_index_daily_${DATE}.log" 2>&1; then
    log "[步骤1/25] ✅ 指数日交易数据 (update_stock_index_daily.py) 执行成功"
else
    log "[步骤1/25] ❌ 指数日交易数据 (update_stock_index_daily.py) 执行失败，停止任务"
    exit 1
fi

# 步骤2：股指期货日交易数据
log "[步骤2/25] 开始执行 股指期货日交易数据 (update_stock_index_future_daily.py)..."
if ${VENV_PYTHON} update_stock_index_future_daily.py >> "${LOG_DIR}/update_stock_index_future_daily_${DATE}.log" 2>&1; then
    log "[步骤2/25] ✅ 股指期货日交易数据 (update_stock_index_future_daily.py) 执行成功"
else
    log "[步骤2/25] ❌ 股指期货日交易数据 (update_stock_index_future_daily.py) 执行失败，停止任务"
    exit 1
fi

# 步骤3：融资融券数据
log "[步骤3/25] 开始执行 融资融券数据 (update_rzrq_ye_daily.py)..."
if ${VENV_PYTHON} update_rzrq_ye_daily.py >> "${LOG_DIR}/update_rzrq_ye_daily_${DATE}.log" 2>&1; then
    log "[步骤3/25] ✅ 融资融券数据 (update_rzrq_ye_daily.py) 执行成功"
else
    log "[步骤3/25] ❌ 融资融券数据 (update_rzrq_ye_daily.py) 执行失败，停止任务"
    exit 1
fi

# 步骤4：个股交易数据
log "[步骤4/25] 开始执行 个股交易数据 (update_stock_daily.py)..."
if ${VENV_PYTHON} update_stock_daily.py >> "${LOG_DIR}/update_stock_daily_${DATE}.log" 2>&1; then
    log "[步骤4/25] ✅ 个股交易数据 (update_stock_daily.py) 执行成功"
else
    log "[步骤4/25] ❌ 个股交易数据 (update_stock_daily.py) 执行失败，停止任务"
    exit 1
fi

# 步骤5：股票基础信息
log "[步骤5/25] 开始执行 股票基础信息 (update_stock_info_daily.py)..."
if ${VENV_PYTHON} update_stock_info_daily.py >> "${LOG_DIR}/update_stock_info_daily_${DATE}.log" 2>&1; then
    log "[步骤5/25] ✅ 股票基础信息 (update_stock_info_daily.py) 执行成功"
else
    log "[步骤5/25] ❌ 股票基础信息 (update_stock_info_daily.py) 执行失败，停止任务"
    exit 1
fi

# 步骤6：股票日基本交易指标数据
log "[步骤6/25] 开始执行 股票日基本交易指标数据 (update_stock_daily_basic_info_daily.py)..."
if ${VENV_PYTHON} update_stock_daily_basic_info_daily.py >> "${LOG_DIR}/update_stock_daily_basic_info_daily_${DATE}.log" 2>&1; then
    log "[步骤6/25] ✅ 股票日基本交易指标数据 (update_stock_daily_basic_info_daily.py) 执行成功"
else
    log "[步骤6/25] ❌ 股票日基本交易指标数据 (update_stock_daily_basic_info_daily.py) 执行失败，停止任务"
    exit 1
fi

# 步骤7：个股MA5/MA30打标
log "[步骤7/25] 开始执行 个股MA5/MA30打标 (update_stock_daily_ma5_ma30.py)..."
if ${VENV_PYTHON} update_stock_daily_ma5_ma30.py >> "${LOG_DIR}/update_stock_daily_ma5_ma30_${DATE}.log" 2>&1; then
    log "[步骤7/25] ✅ 个股MA5/MA30打标 (update_stock_daily_ma5_ma30.py) 执行成功"
else
    log "[步骤7/25] ❌ 个股MA5/MA30打标 (update_stock_daily_ma5_ma30.py) 执行失败，停止任务"
    exit 1
fi

# 步骤8：个股turning_point打标
log "[步骤8/25] 开始执行 个股turning_point打标 (update_stock_daily_turning_point.py)..."
if ${VENV_PYTHON} update_stock_daily_turning_point.py >> "${LOG_DIR}/update_stock_daily_turning_point_${DATE}.log" 2>&1; then
    log "[步骤8/25] ✅ 个股turning_point打标 (update_stock_daily_turning_point.py) 执行成功"
else
    log "[步骤8/25] ❌ 个股turning_point打标 (update_stock_daily_turning_point.py) 执行失败，停止任务"
    exit 1
fi

# 步骤9：股票技术面指标数据
log "[步骤9/25] 开始执行 股票技术面指标数据 (update_stock_daily_factor.py)..."
if ${VENV_PYTHON} update_stock_daily_factor.py >> "${LOG_DIR}/update_stock_daily_factor_${DATE}.log" 2>&1; then
    log "[步骤9/25] ✅ 股票技术面指标数据 (update_stock_daily_factor.py) 执行成功"
else
    log "[步骤9/25] ❌ 股票技术面指标数据 (update_stock_daily_factor.py) 执行失败，停止任务"
    exit 1
fi

# 步骤10：全市场短线强弱得分回填
log "[步骤10/25] 开始执行 全市场短线强弱得分回填 (update_stock_daily_short_strength.py)..."
if ${VENV_PYTHON} update_stock_daily_short_strength.py >> "${LOG_DIR}/update_stock_daily_short_strength_${DATE}.log" 2>&1; then
    log "[步骤10/25] ✅ 全市场短线强弱得分回填 (update_stock_daily_short_strength.py) 执行成功"
else
    log "[步骤10/25] ❌ 全市场短线强弱得分回填 (update_stock_daily_short_strength.py) 执行失败，停止任务"
    exit 1
fi
...
if ${VENV_PYTHON} update_strategy_selected_stock_daily.py >> "${LOG_DIR}/update_strategy_selected_stock_daily_${DATE}.log" 2>&1; then
    ...
    log "[步骤13/25] ❌ 选股结果回测指标回填 (update_strategy_selected_stock_daily.py) 执行失败，停止任务"
    exit 1
fi
...
# 步骤15：V形反转策略
log "[步骤15/25] 开始执行 V形反转策略 (select_v_reverse.py)..."
if ${VENV_PYTHON} select_v_reverse.py >> "${LOG_DIR}/select_v_reverse_${DATE}.log" 2>&1; then
    log "[步骤15/25] ✅ V形反转策略 (select_v_reverse.py) 执行成功"
else
    log "[步骤15/25] ❌ V形反转策略 (select_v_reverse.py) 执行失败，停止任务"
    exit 1
fi

# 步骤16：二浪启动策略
log "[步骤16/25] 开始执行 二浪启动策略 (select_2wave_up.py)..."
if ${VENV_PYTHON} select_2wave_up.py >> "${LOG_DIR}/select_2wave_up_${DATE}.log" 2>&1; then
    log "[步骤16/25] ✅ 二浪启动策略 (select_2wave_up.py) 执行成功"
else
    log "[步骤16/25] ❌ 二浪启动策略 (select_2wave_up.py) 执行失败，停止任务"
    exit 1
fi
...
# 步骤19：W23二浪选股策略
log "[步骤19/25] 开始执行 W23二浪选股策略 (select_2wave_w23.py)..."
if ${VENV_PYTHON} select_2wave_w23.py >> "${LOG_DIR}/select_2wave_w23_${DATE}.log" 2>&1; then
    log "[步骤19/25] ✅ W23二浪选股策略 (select_2wave_w23.py) 执行成功"
else
    log "[步骤19/25] ❌ W23二浪选股策略 (select_2wave_w23.py) 执行失败，停止任务"
    exit 1
fi
...
# 步骤20：选股结果回测回填
log "[步骤20/25] 开始执行 选股结果回测回填 (backtest_strategy_results.py)..."
if ${VENV_PYTHON} backtest_strategy_results.py >> "${LOG_DIR}/backtest_strategy_results_${DATE}.log" 2>&1; then
    log "[步骤20/25] ✅ 选股结果回测回填 (backtest_strategy_results.py) 执行成功"
else
    log "[步骤20/25] ❌ 选股结果回测回填 (backtest_strategy_results.py) 执行失败，停止任务"
    exit 1
fi

# 步骤21：策略结果分析统计
log "[步骤21/25] 开始执行 策略结果分析统计 (calc_strategy_result_analysis.py)..."
if ${VENV_PYTHON} calc_strategy_result_analysis.py >> "${LOG_DIR}/calc_strategy_result_analysis_${DATE}.log" 2>&1; then
    log "[步骤21/25] ✅ 策略结果分析统计 (calc_strategy_result_analysis.py) 执行成功"
else
    log "[步骤21/25] ❌ 策略结果分析统计 (calc_strategy_result_analysis.py) 执行失败，停止任务"
    exit 1
fi
...
# 步骤22：大盘指标体系
log "[步骤22/25] 开始执行 大盘指标体系 (report_market_overview_metricx.py)..."
if ${VENV_PYTHON} report_market_overview_metricx.py >> "${LOG_DIR}/report_market_overview_metricx_${DATE}.log" 2>&1; then
    log "[步骤22/25] ✅ 大盘指标体系 (report_market_overview_metricx.py) 执行成功"
else
    log "[步骤22/25] ❌ 大盘指标体系 (report_market_overview_metricx.py) 执行失败，停止任务"
    exit 1
fi

# 步骤23：A股大盘风险指数MRI
log "[步骤23/25] 开始执行 A股大盘风险指数MRI (report_market_risk_metricx.py)..."
if ${VENV_PYTHON} report_market_risk_metricx.py >> "${LOG_DIR}/report_market_risk_metricx_${DATE}.log" 2>&1; then
    log "[步骤23/25] ✅ A股大盘风险指数MRI (report_market_risk_metricx.py) 执行成功"
else
    log "[步骤23/25] ❌ A股大盘风险指数MRI (report_market_risk_metricx.py) 执行失败，停止任务"
    exit 1
fi
...
# 步骤24：数据更新监控
log "[步骤24/25] 开始执行 数据更新监控 (monitor_stock_data.py)..."
if ${VENV_PYTHON} monitor_stock_data.py >> "${LOG_DIR}/monitor_stock_data_${DATE}.log" 2>&1; then
    log "[步骤24/25] ✅ 数据更新监控 (monitor_stock_data.py) 执行成功"
else
    log "[步骤24/25] ❌ 数据更新监控 (monitor_stock_data.py) 执行失败，停止任务"
    exit 1
fi

# 步骤25：任务运行监控
log "[步骤25/25] 开始执行 任务运行监控 (monitor_run_daily_tasks.py)..."
if ${VENV_PYTHON} monitor_run_daily_tasks.py >> "${LOG_DIR}/monitor_run_daily_tasks_${DATE}.log" 2>&1; then
    log "[步骤25/25] ✅ 任务运行监控 (monitor_run_daily_tasks.py) 执行成功"
else
    log "[步骤25/25] ❌ 任务运行监控 (monitor_run_daily_tasks.py) 执行失败，停止任务"
    exit 1
fi
