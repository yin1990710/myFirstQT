...
# 步骤19：选股结果回测回填
log "[步骤19/25] 开始执行 选股结果回测回填 (backtest_strategy_results.py)..."
if ${VENV_PYTHON} backtest_strategy_results.py >> "${LOG_DIR}/backtest_strategy_results_${DATE}.log" 2>&1; then
    log "[步骤19/25] ✅ 选股结果回测回填 (backtest_strategy_results.py) 执行成功"
else
    log "[步骤19/25] ❌ 选股结果回测回填 (backtest_strategy_results.py) 执行失败，停止任务"
    exit 1
fi

# 步骤20：策略结果分析统计
log "[步骤20/25] 开始执行 策略结果分析统计 (calc_strategy_result_analysis.py)..."
if ${VENV_PYTHON} calc_strategy_result_analysis.py >> "${LOG_DIR}/calc_strategy_result_analysis_${DATE}.log" 2>&1; then
    log "[步骤20/25] ✅ 策略结果分析统计 (calc_strategy_result_analysis.py) 执行成功"
else
    log "[步骤20/25] ❌ 策略结果分析统计 (calc_strategy_result_analysis.py) 执行失败，停止任务"
    exit 1
fi
...
# 步骤21：大盘指标体系
log "[步骤21/25] 开始执行 大盘指标体系 (report_market_overview_metricx.py)..."
if ${VENV_PYTHON} report_market_overview_metricx.py >> "${LOG_DIR}/report_market_overview_metricx_${DATE}.log" 2>&1; then
    log "[步骤21/25] ✅ 大盘指标体系 (report_market_overview_metricx.py) 执行成功"
else
    log "[步骤21/25] ❌ 大盘指标体系 (report_market_overview_metricx.py) 执行失败，停止任务"
    exit 1
fi

# 步骤22：A股大盘风险指数MRI
log "[步骤22/25] 开始执行 A股大盘风险指数MRI (report_market_risk_index.py)..."
if ${VENV_PYTHON} report_market_risk_index.py >> "${LOG_DIR}/report_market_risk_index_${DATE}.log" 2>&1; then
    log "[步骤22/25] ✅ A股大盘风险指数MRI (report_market_risk_index.py) 执行成功"
else
    log "[步骤22/25] ❌ A股大盘风险指数MRI (report_market_risk_index.py) 执行失败，停止任务"
    exit 1
fi
...
# 步骤23：行业短线热度分析
log "[步骤23/25] 开始执行 行业短线热度分析 (report_industry_heat.py)..."
if ${VENV_PYTHON} report_industry_heat.py >> "${LOG_DIR}/report_industry_heat_${DATE}.log" 2>&1; then
    log "[步骤23/25] ✅ 行业短线热度分析 (report_industry_heat.py) 执行成功"
else
    log "[步骤23/25] ❌ 行业短线热度分析 (report_industry_heat.py) 执行失败，停止任务"
    exit 1
fi
