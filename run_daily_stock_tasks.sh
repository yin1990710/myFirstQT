...
# 步骤25：东财行业/概念板块当日快照
log "[步骤25/28] 开始执行 东财行业/概念板块快照 (update_em_board_daily.py)..."
if ${VENV_PYTHON} update_em_board_daily.py >> "${LOG_DIR}/update_em_board_daily_${DATE}.log" 2>&1; then
    log "[步骤25/28] ✅ 东财行业/概念板块快照 (update_em_board_daily.py) 执行成功"
else
    log "[步骤25/28] ❌ 东财行业/概念板块快照 (update_em_board_daily.py) 执行失败，停止任务"
    exit 1
fi

# 步骤26：行业短线热度分析
log "[步骤26/28] 开始执行 行业短线热度分析 (report_industry_heat.py)..."
if ${VENV_PYTHON} report_industry_heat.py >> "${LOG_DIR}/report_industry_heat_${DATE}.log" 2>&1; then
    log "[步骤26/28] ✅ 行业短线热度分析 (report_industry_heat.py) 执行成功"
else
    log "[步骤26/28] ❌ 行业短线热度分析 (report_industry_heat.py) 执行失败，停止任务"
    exit 1
fi
