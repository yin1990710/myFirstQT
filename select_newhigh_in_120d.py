...
1. 最近1个交易日收盘价（close），超过最近120天（不含最近一天）的最高股价（high）。
2. 最近1个交易日的涨幅（(close-前一日收盘价)/前一日收盘价）超过8%。
...
def analyze_newhigh_stocks(df: pd.DataFrame) -> list:
    ...
    for ts_code, group in grouped:
        ...
        max_high_119 = previous_119_days['high'].max()

        if max_high_119 == 0:
            continue

        if latest['close'] <= max_high_119:
            continue
...
def main():
    ...
    if results:
        print("\n📊 符合条件的股票列表:")
        print("-" * 80)
        print(f"{'股票代码':<12} {'股票名称':<15} {'最新收盘':<10} {'120日最高':<10} {'突破幅度':<10} {'涨幅(%)':<10}")
        print("-" * 80)

        for r in results[:20]:
            print(f"{r['ts_code']:<12} {r['stock_name']:<15} "
                  f"{r['latest_close']:>8.2f}   "
                  f"{r['max_high_119']:>8.2f}   "
